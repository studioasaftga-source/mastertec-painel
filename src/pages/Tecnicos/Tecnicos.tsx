...
export default function Tecnicos() {
  ...
  const [modoEdicaoId, setModoEdicaoId] =
  ...
  const carregarTecnicos = useCallback(async () => {
    console.log('DEBUG TÉCNICOS - INÍCIO:', {
      usuario,
      idUsuario: usuario?.id,
      empresaDoUsuario: usuario?.empresa_id,
      role: usuario?.role,
      ativo: usuario?.ativo,
      administracaoPermitida,
    })

    if (!usuario?.empresa_id || !administracaoPermitida) {
      console.log(
        'DEBUG TÉCNICOS - BLOQUEADO ANTES DA CONSULTA:',
        {
          empresaDoUsuario: usuario?.empresa_id,
          administracaoPermitida,
        },
    ...
    try {
      ...
      console.log(
        'DEBUG TÉCNICOS - CONSULTANDO USUÁRIOS:',
        {
      ...
      const { data, error } = await supabase
        .from('usuarios')
        .select(
          'id,empresa_id,nome,codigo_acesso,percentual_comissao,ativo',
        )
        .eq('empresa_id', usuario.empresa_id)
        .order('ativo', { ascending: false })
        .order('nome', { ascending: true })

      console.log('DEBUG TÉCNICOS - RESULTADO:', {
        empresaDoUsuario: usuario.empresa_id,
  ...
  async function salvarTecnico() {
    ...
    try {
      ...
      if (modoEdicaoId) {
        ...
        const tecnicoCriado = Array.isArray(data)
          ? data[0]
          : data

        const codigo =
          tecnicoCriado?.codigo_acesso
  ...
  async function alternarAtivo(
    ...
    if (!administracaoPermitida || salvando) {
      return
    }

    const desejaAtivar =
      !Boolean(tecnico.ativo)
    ...
    try {
      ...
      console.error(
        'Erro ao alterar status do técnico:',
        error,
      )
      ...
      <div style={styles.page}>
        <div style={styles.headerRow}>
          <div>
            ...
            <p style={styles.muted}>
              Cadastre o técnico e defina a
              porcentagem usada nas próximas O.S.
            </p>
          </div>
